  ...
  return (
    <>
      ...
      <header className="flex flex-wrap items-center justify-between gap-4 mb-8">
        <h1 className="text-[49px] font-bold">{isNew ? "행동 추가" : action.name}</h1>
        {!isNew && (
          <span className="inline-block text-[22px] font-bold bg-accentsoft text-accent rounded-full px-4 py-2">
      ...
      <section className="bg-surface border border-line rounded-(--radius-card) px-8 py-7 mb-8">
        ...
        <Field label="필요한 정보" help="동행이가 이 행동을 실행할 때 채워 넣는 값입니다.">
          <div className="flex flex-col gap-2">
            ...
            <Button variant="ghost" className="self-start h-12 text-[22px]"
        ...
        {isNew ? (
          ...
          <p className="text-muted text-[22px] mb-5">
      ...
      {!isNew && (
        <section className="bg-surface border border-line rounded-(--radius-card) px-8 py-7 mb-8">
          <h2 className="text-[28px] font-bold mb-2">이 행동을 쓰는 시나리오</h2>
          {usedBy.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2.5">
              {usedBy.map((s) => (
                <Link key={s.id} to={`/scenarios/${s.id}`}
                      className="inline-block text-[22px] font-bold bg-accentsoft text-accent rounded-full px-5 py-2.5 hover:underline">
                  {s.name}
                </Link>
              ))}
            </div>
