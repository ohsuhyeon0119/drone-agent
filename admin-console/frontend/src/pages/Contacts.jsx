  ...
  return (
    <>
      ...
      <form onSubmit={submit} className="bg-surface border border-line rounded-(--radius-card) p-5 mb-6">
        ...
        {error && <p role="alert" className="text-warn text-[14px] mb-3">{error}</p>}
      ...
      {draft.contacts.length === 0 ? (
        ...
        <ul className="flex flex-col gap-3 mb-8">
          {draft.contacts.map((c, i) => (
            ...
            >
              <span
                className={`flex-none w-16 text-center text-[12px] font-bold rounded-full px-2 py-1
                  ${i === 0 ? "bg-accentsoft text-accent" : "bg-bg text-muted"}`}
              >
                {i + 1}순위
              </span>
              <div className="flex-1 min-w-[77px]">
                <span className="font-bold text-[17px]">{c.name}</span>
                <span className="text-muted text-[14px]"> · {c.relation}</span>
                <div className="text-ink/75 text-[15px] tabular-nums">{c.phone}</div>
              </div>
