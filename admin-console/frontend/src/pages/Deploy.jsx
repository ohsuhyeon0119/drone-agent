    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        <div className="bg-surface border border-line rounded-(--radius-card) p-7">
          <div className="text-[22px] tracking-wide text-muted mb-1.5">현재 적용 버전</div>
          <div className="text-[35px] font-bold tabular-nums">Live v{live.version}</div>
          <div className="text-muted text-[24px] tabular-nums mt-1">{live.publishedAt} 배포됨</div>
        </div>
        <div className={`rounded-(--radius-card) p-7 border ${changes.length ? "bg-warnsoft border-warn/40" : "bg-surface border-line"}`}>
          <div className="text-[22px] tracking-wide text-muted mb-1.5">편집 중</div>
          <div className="text-[35px] font-bold">
            {changes.length ? <span className="text-warn">수정 {changes.length}건</span> : "변경 없음"}
          </div>
          <div className="text-muted text-[24px]">배포 후 적용됩니다</div>
        </div>
      </div>

      <section className="mb-8">
        <h2 className="text-[32px] font-bold mb-4">변경 사항</h2>
        {changes.length === 0 ? (
          ...
          <>
            <ul className="flex flex-col gap-2 mb-5">
              {changes.map((c, i) => (
                <li
                  key={i}
                  className="bg-surface border border-line rounded-(--radius-card) border-l-4 border-l-accent px-7 py-6 text-[26px] leading-relaxed"
                >
                  {c}
                </li>
              ))}
      ...
      <section>
        <h2 className="text-[32px] font-bold mb-4">배포 기록</h2>
        <div className="overflow-x-auto bg-surface border border-line rounded-(--radius-card)">
          <table className="w-full min-w-[540px] border-collapse">
            <thead>
              <tr className="text-left text-[22px] tracking-wide text-muted border-b border-line">
            ...
            <tbody>
              {[...versions].reverse().map((v) => (
                <tr key={v.version} className="border-b border-line last:border-b-0 align-top">
                  <td className="px-7 py-7 font-bold text-[28px] tabular-nums whitespace-nowrap">
                    v{v.version}
                    {v.version === live.version && (
                      <span className="ml-3 text-[22px] font-bold bg-accentsoft text-accent rounded-full px-2.5 py-0.5 align-middle">Live</span>
                    )}
                  </td>
                  <td className="px-7 py-7 text-ink text-[26px] tabular-nums whitespace-nowrap">{v.publishedAt}</td>
                  <td className="px-7 py-7 text-[26px] text-ink leading-relaxed">
