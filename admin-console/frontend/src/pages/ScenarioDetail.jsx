  ...
  return (
    <>
      ...
      <header className="flex flex-wrap items-center justify-between gap-4 mb-8">
        <h1 className="text-[27px] font-bold">{scenario.name}</h1>
        <div className="flex items-center gap-3">
          <span className="text-[14px] text-muted">{scenario.enabled ? "감시 중" : "꺼짐"}</span>
          <Toggle
      ...
      <section className="mb-10 bg-surface border border-line rounded-(--radius-card) px-7 py-6">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-2">
          <h2 className="text-[19px] font-bold">감지 조건</h2>
          <div className="flex items-center gap-3">
            <span className="text-[13px] text-muted">몇 초마다 판단할까요</span>
            <select
              aria-label="재판정 간격"
              className="h-14 px-4 text-[14px] rounded-(--radius-ctl) border border-line bg-surface cursor-pointer tabular-nums"
        ...
        {scenario.detectPrompt ? (
          ...
          <details className="mt-3">
            <summary className="text-muted text-[12px] cursor-pointer select-none">
              카메라에 실제로 전달되는 전체 문장 보기
            </summary>
            <pre className="mt-2 p-4 rounded-(--radius-ctl) bg-bg border border-line text-[11px] leading-relaxed whitespace-pre-wrap font-sans text-muted">
{scenario.detectPrompt}
...
{`{"event": "${scenario.id === "fall" ? "fall" : "taken"}" 또는 "none", "confidence": 0.0~1.0, "reason": "판단 근거를 한 문장으로"}`}
      ...
      <section className="mb-10">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <div>
            <h2 className="text-[19px] font-bold">지침</h2>
            <p className="text-muted text-[13px] mt-1">감지된 뒤 동행이가 어떻게 말하고 행동할지</p>
          </div>
            ...
            {scenario.instructions.map((ins, i) => (
              ...
              >
                <div className="flex items-start gap-4">
                  <p className="flex-1 text-[17px] leading-relaxed">{ins.text}</p>
                ...
                <div className="flex flex-wrap items-center gap-3 mt-4 pt-4 border-t border-line/70">
                  <span className="text-[13px] text-muted">이때 실행할 행동</span>
                  <select
                    aria-label={`지침 ${i + 1}의 행동`}
                    className="h-14 px-4 text-[14px] rounded-(--radius-ctl) border border-line bg-surface cursor-pointer"
                    value={ins.action || ""}
                  ...
                  {ins.action && (
                    <Link to={`/actions/${ins.action}`}
                          className="text-accent font-semibold text-[13px] hover:underline">
                      연락 대상·설정 보기 →
                    </Link>
                  )}
                </div>
