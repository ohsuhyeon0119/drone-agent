...
낙상·복약 두 시나리오는 `agent/scenarios/*.yaml`로 구조화돼 있어 코드 수정
없이 감지 프롬프트·쿨다운·지침을 고칠 수 있다(자세한 내용은
[`agent/README.md`](agent/README.md)). 파일이 없거나 잘못돼도
`agent/scenarios.py`의 기본값으로 안전하게 대체된다. tool(`notify_caregiver`)은
아직 코드에 하드코딩돼 있다. 다음 단계로 논의된 것(설계만 하고 아직 미구현):

- **시나리오/tool 레지스트리**: 지금은 전역 설정 파일(`agent/scenarios/`)
  하나뿐인데, 이걸 `users/<id>.json`처럼 노인별로 나눠서 사람마다 다른
  시나리오 조합을 켜고 끌 수 있게 함. tool 목록도 같은 방식으로 사용자별
  구성. (단, Gemini Live는 tool을 세션 시작 시점에만 고정할 수 있어 — 설정을
  바꾸면 다음 재연결부터 반영됨)
