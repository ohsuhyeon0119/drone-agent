...
class GeminiLive:
    ...
    def __init__(self, api_key, model, input_sample_rate, tools=None, tool_mapping=None, system_instruction=None):
        ...
        Args:
            ...
            system_instruction (str, optional): Persona/scenario system prompt. Falls back to a
                generic elderly-care drone assistant persona if not provided.
        """
        ...
        self.system_instruction = system_instruction or (
            "당신은 노인 케어 AI 드론 어시스턴트입니다. 노인분들이 일상생활에서 도움이 "
            "필요할 때 친절하고 명확하게 안내하고 대화합니다."
        )

    async def start_session(self, audio_input_queue, video_input_queue, text_input_queue, audio_output_callback, audio_interrupt_callback=None, nudge_input_queue=None):
        ...
        try:
          async with self.client.aio.live.connect(model=self.model, config=config) as session:
            ...
            async def send_nudge():
                """비전 감지 등 외부 트리거로 강제로 한 턴을 발생시킨다.

                send_realtime_input은 오디오 VAD가 켜져 있으면 activity_end를
                직접 못 보내므로(서버 자동 감지 전용 모드), 대신 턴 단위로
                확정 응답을 요청하는 send_client_content(turn_complete=True)를 쓴다.
                """
                if nudge_input_queue is None:
                    return
                try:
                    while True:
                        nudge_text = await nudge_input_queue.get()
                        logger.info(f"Forcing proactive turn with nudge: {nudge_text}")
                    ...
                    logger.debug("send_nudge task cancelled")
                except Exception as e:
                    logger.error(f"send_nudge error: {e}\n{traceback.format_exc()}")
            ...
            send_nudge_task = asyncio.create_task(send_nudge())
